...
function toggleDark() {
  ...
  if (darkMode[0].className.match(/(?:^|\s)show(?!\S)/)) {
    for (var index = 0; index < light.length; index++)
      light[index].className = light[index].className.replace(
        /(?:^|\s)darkback(?!\S)/g,
        ""
      );
    document.getElementsByClassName(
      ...
      .getElementsByClassName("mainhead")[0]
      .className.replace(/(?:^|\s)darkback(?!\S)/g, "");
    document.getElementById("titlename").className = document
